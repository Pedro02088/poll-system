...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/reset-password', { ...form, token, email })
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-fade-in relative">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-xl shadow-blue-500/5 border border-slate-100">
          <h2 className="text-xl font-bold text-slate-800 mb-2 font-display">Nova senha</h2>
          <p className="text-slate-500 text-sm mb-6">Defina a nova senha para <strong className="text-slate-700">{email}</strong>.</p>
          ...
          <Input label="Nova senha" type="password" value={form.password}
            onChange={(e) => setForm({ ...form, password: e.target.value })} required />
          <Input label="Confirmar nova senha" type="password" value={form.password_confirmation}
            onChange={(e) => setForm({ ...form, password_confirmation: e.target.value })} required />
          <Button type="submit" disabled={loading || !token}>{loading ? 'Redefinindo...' : 'Redefinir senha'}</Button>
